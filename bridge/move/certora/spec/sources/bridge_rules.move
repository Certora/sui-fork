...
module spec::bridge_rules {
	use bridge::{
		...
		bridge_env::{get_total_supply, get_total_supply_if_registered},
		message::{BridgeMessage, get_transfer_payload},
		message_types
	};
	use certora::sui_object_summaries::deleted;
	use cvlm::{
		asserts::{cvlm_assert, cvlm_assert_msg, cvlm_assume_msg},
		function::Function,
		ghost::ghost_destroy,
		manifest::{rule, target, invoker, target_sanity},
		nondet::nondet
	};
	use sui::{address, clock::Clock, coin::{Coin, value}, event::events_by_type};
	...
	public fun only_owner_can_claim<T>(
		...
		let msg = bridge
			.test_get_parsed_token_transfer_message(source_chain, bridge_seq_num)
			.destroy_some();
	...
	public fun transfer_records_are_valid(
		bridge: &mut Bridge,
		fn: Function,
		ctx: &mut TxContext,
		state: &mut SuiSystemState,
	...
	public fun approve_token_transfer_effects(
		...
		cvlm_assume_msg(
			invariant_pending_status_only_for_internal_transfers(
				bridge,
				message.source_chain(),
				message.seq_num(),
			),
			b"invariant_pending_status_only_for_internal_transfers",
		);

		let statusBefore = bridge.test_get_token_transfer_action_status(
			message.source_chain(),
			message.seq_num(),
		);
		bridge.approve_token_transfer(message, signatures);
		let statusAfter = bridge.test_get_token_transfer_action_status(
			message.source_chain(),
			message.seq_num(),
		);
		...
		if (approved_events.length() ==1) {
			cvlm_assert(
				statusBefore == transfer_status_not_found() || statusBefore == transfer_status_pending(),
			);
			cvlm_assert(statusAfter == transfer_status_approved());
		} else {
			cvlm_assert(
				statusBefore == transfer_status_approved() || statusBefore == transfer_status_claimed(),
			);
	...
	public fun transfer_record_status_changes(
		bridge: &mut Bridge,
		source_chain: u8,
		bridge_seq_num: u64,
		fn: Function,
	...
	public fun only_approve_token_transfer_can_approve_transfer(
		...
		if (fn.name() != b"approve_token_transfer") {
			cvlm_assert(
				statusBefore == transfer_status_approved() || statusAfter != transfer_status_approved(),
			);
		}
	}

	/// Check that all pending transfers must be internal transfers (source_chain == inner.chain_id).
	fun invariant_pending_status_only_for_internal_transfers(
		bridge: &mut Bridge,
		source_chain: u8,
		bridge_seq_num: u64,
	): bool {
	...
	/// Check that all pending transfers must be internal transfers (source_chain == inner.chain_id).
	public fun check_invariant_pending_status_only_for_internal_transfers(
		bridge: &mut Bridge,
		source_chain: u8,
		bridge_seq_num: u64,
		fn: Function,
		...
		cvlm_assume_msg(
			invariant_pending_status_only_for_internal_transfers(bridge, source_chain, bridge_seq_num),
			b"invariant holds before",
		);
		invoke(fn, bridge, ctx, state);
		cvlm_assert_msg(
			invariant_pending_status_only_for_internal_transfers(bridge, source_chain, bridge_seq_num),
			b"invariant holds after",
		);
	}
}
